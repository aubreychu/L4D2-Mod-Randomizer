...
async def get_collection_items(session, collection_id):
    ...
    try:
        async with session.get(url, cookies=cookies, timeout=10) as response:
            if response.status == 200:
                ...
                matches = re.findall(r'id="sharedfile_(\d+)"', text)
                if matches:
    ...
    try:
        ...
        async with session.post(api_url, data=data, timeout=10) as response:
            if response.status == 200:
                ...
                details = json_data.get("response", {}).get("collectiondetails", [])
                if details and "children" in details[0]:
                    return [
                        child["publishedfileid"] for child in details[0]["children"]
                    ]
    except Exception:
        pass
...
async def async_modify_collection(session, collection_id, mod_id, action="addchild"):
    ...
    cookies = {
        ...
        "steamLoginSecure": USER_CONFIG["STEAM_LOGIN_SECURE"],
    }
    data = {
        "sessionid": USER_CONFIG["SESSION_ID"],
        "id": collection_id,
...
async def resolve_dependencies_recursive(session, api_key, mod_ids, visited=None):
    ...
    for i in range(0, len(to_check), 100):
        ...
        params = {"key": api_key, "include_children": "true"}
        ...
        try:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if new_children:
                        sub_deps = await resolve_dependencies_recursive(
                            session, api_key, new_children, visited
                        )
