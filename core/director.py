...
def prep_mixed_pool(
    ...
    try:
        if cache_ratio == 1.0:
            ...
            if progress_callback:
                progress_callback("Skipping Network Scrape (Using Local Cache)...", 0.5)
        else:
    ...
    if f_type != FilterType.NONE:
        ...
        log.info(
            f"Filter [{f_type.value}] active. Isolated the top {len(cached_mods)} elite mods."
        )
    else:
    ...
    stats = {
        ...
        "cached_pct": (
            int((len(cached_mods) / total_pool_size) * 100)
            if total_pool_size > 0
            else 0
        ),
        "new_pct": (
            int((len(new_mods) / total_pool_size) * 100) if total_pool_size > 0 else 0
        ),
    }
    ...
    if actual_ratio == 1.0:
        mixed_pool = cached_mods
    elif actual_ratio == 0.0:
        mixed_pool = new_mods
    else:
...
def allocate_loadout(mixed_pool):
    assignments = {
        f"{s} [{t}]": None for s in VPK_DICT.keys() for t in ["Model", "Sound"]
    }
        ...
        for slot_type in ["Model", "Sound"]:
            ...
            if assignments[target_key] is None:
                for mod in mixed_pool:
                    if mod.id in selected_mods:
                        continue

                    fits = False
                    if slot_type == "Model" and slot_name in mod.eval.model_slots:
                        fits = True
                    elif slot_type == "Sound" and slot_name in mod.eval.audio_slots:
                        fits = True

                    if fits:
                        conflict = False
                        for ms in mod.eval.model_slots:
                            if assignments.get(f"{ms} [Model]") is not None:
                                conflict = True
                        for ast in mod.eval.audio_slots:
                            if assignments.get(f"{ast} [Sound]") is not None:
                                conflict = True
...
def reroll_single_slot(
    full_slot_name, mixed_pool, current_assignments, current_selected
):
    ...
    for mod in mixed_pool:
        if mod.id in current_selected:
            continue

        fits = False
        if slot_type == "Model" and base_slot in mod.eval.model_slots:
            fits = True
        elif slot_type == "Sound" and base_slot in mod.eval.audio_slots:
            fits = True
...
async def _async_sync_pipeline(collection_id, selected_ids, progress_callback):
    ...
    async def safe_modify(session, cid, mid, action):
        async with semaphore:
            ...
            if not result:
                log.warning(
                    f"Steam API rejected {action} for Mod ID {mid}. (Possible rate limit or dead link)"
                )
            return result

    async with aiohttp.ClientSession() as session:
        if progress_callback:
            progress_callback("Reading current collection...", 0.1)
        current_mods = await get_collection_items(session, collection_id)

        if not current_mods:
            log.warning(
                "Collection appears empty. (If it's not, verify your Session ID cookies)."
            )
        else:
            log.info(
                f"Targeting {len(current_mods)} items currently in the collection for wipe processing."
            )
        ...
        if api_key:
            if progress_callback:
                progress_callback("Resolving mod dependencies...", 0.2)
            try:
                deps = await resolve_dependencies_recursive(
                    session, api_key, list(mods_to_add)
                )
                if deps:
            ...
            log.warning(
                "STEAM_API_KEY missing. Cannot automatically resolve dependencies."
            )

        mods_to_remove = [m for m in current_mods if m not in mods_to_add]
        if progress_callback:
            progress_callback(f"Clearing {len(mods_to_remove)} old mods...", 0.4)

        if mods_to_remove:
            remove_tasks = [
                safe_modify(session, collection_id, m, "removechild")
                for m in mods_to_remove
            ]
            await asyncio.gather(*remove_tasks)

        if progress_callback:
            progress_callback(f"Injecting {len(mods_to_add)} mods...", 0.7)

        if mods_to_add:
            add_tasks = [
                safe_modify(session, collection_id, m, "addchild") for m in mods_to_add
            ]
...
def sync_collection_loadout(collection_id, selected_ids, progress_callback=None):
    ...
    loop.run_until_complete(
        _async_sync_pipeline(collection_id, selected_ids, progress_callback)
    )
